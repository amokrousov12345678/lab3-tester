...
def current_time_seconds(): return int(round(time.time()))
...
error_codes = {ESUCCESS: "OK", EDUPMSG: "Msg duplicated", ELOSTMSG: "Msg lost",
               EEXTRAMSG: "Unexpected message in output", EINVALIDTEST: "Test is invalid"}
...
def do_test_cmd(cmd_params, nodes, fd_to_stream, fd_to_nodeid, verbosity, send_msg):
    ...
    if cmd_params[0] == "add" or cmd_params[0] == "add_instant":
        ...
        nodes[my_id] = subprocess.Popen(
            cur_params, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
...
def check_test(fname, verbosity):
    ...
    with open(fname) as f:
        ...
        for line in f:
            ...
            verdict = do_test_cmd(
                cmd_params, nodes, fd_to_stream, fd_to_nodeid, verbosity, send_msg)
            if (verbosity == VERBOSE):
